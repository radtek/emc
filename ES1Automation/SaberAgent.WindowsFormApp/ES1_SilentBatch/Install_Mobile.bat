...
::
:: DESCRIPTION
::   	Silently installs the Mobile Services.
::
:: LAST UPDATED 
...
::
:: USAGE
:: 	SILENT MODE : Command Prompt> Install_Mobile.bat
::	
...
::Set default values
IF %BATCHLOG%=="" 	SET BATCHLOG=%MOBILE_BATCHLOG%
IF %INSTALLLOG%=="" 	SET INSTALLLOG=%MOBILE_INSTALLLOG%
...
::Create batch log
ECHO SourceOne Mobile Services Install Log >>%BATCHLOG%
ECHO. >>%BATCHLOG%
...
@ECHO Installing Mobile Services.  Please wait...
ECHO Mobile Services Installation >>%BATCHLOG%
ECHO   Started at %DATE% %TIME% >>%BATCHLOG%

ES1_MobileSetup.exe /s /v"/qn /L*v %INSTALLLOG% INSTALLDIR=%INSTALLDIR% EX_NET_TCPSERVER=%EX_WEBSVCS% EX_TF_DOMAIN=%EX_DOMAIN% EX_TF_USERNAME=%EX_USER% EX_TF_PASSWORD=%EX_PASS% EX_TF_GROUP=%EX_GRP_SEC% REBOOT=ReallySuppress "
